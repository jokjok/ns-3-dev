...
#include "netmap-fd-reader.h"

NS_LOG_COMPONENT_DEFINE ("NetmapFdReader");

namespace ns3 {

NetmapFdReader::NetmapFdReader ()
...
void NetmapFdReader::Start (Callback<void, uint8_t *, ssize_t> readCallback)
{
  NS_LOG_FUNCTION (this << m_fd << &readCallback);
  int tmp;
  ...
  if (!m_destroyEvent.IsRunning ())
    {
      ...
      this->Ref ();
      m_destroyEvent =
        Simulator::ScheduleDestroy (&NetmapFdReader::DestroyEvent, this);
    }
  ...
  m_readThread = Create<SystemThread> (MakeCallback (&NetmapFdReader::Run, this));
  m_readThread->Start ();
}

void NetmapFdReader::DestroyEvent (void)
{
  NS_LOG_FUNCTION (this);
  Stop ();
  this->Unref ();
  ...
  for (;;)
    {
      ...
      if (FD_ISSET (m_fd, &readfds))
        {
          struct NetmapFdReader::Data data = DoRead ();
